...
function isPatientInfoSection(sectionTitle = "") {
  return sectionTitle === "Patient" || sectionTitle.startsWith("Patient and Visit");
}

function isRepeatedDemographicField(field, sectionTitle = "") {
...
function demographicAutofillValue(field, answers) {
  const id = String(field.id || "").toLowerCase();
  const label = String(field.label || "").toLowerCase().trim();
...
function isStaffOnlyField(field) {
...
function fieldOwner(field, sectionTitle) {
